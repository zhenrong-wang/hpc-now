...
/** 
 * This code is only for GNU/Linux distributions to build the HPC Manager toolset. 
 * It is not necessary to consider cross-platform, because the HPC Manager will 
 * only run on the Master nodes of the clusters.
 */
...
/**
 * @brief
 *  Check whether the appstore environment variables are already set int the file
 *  /usr/hpc-now/appstore_env.sh
 *
 * @param[in] void
 *
 * @return int
 *  1 if the environment variables are absent
 *  0 if the envrionment variables are set
 *
 */
...
/**
 * @brief
 *  Print out the header of the hpcmgr. This function is *deprecated*.
 *  
 * @param[in] void
 *
 * @return void
...
/**
 * @brief
 *  Print out the tail of the hpcmgr. This function is *deprecated*.
 *  
 * @param[in] void
 *
 * @return void
...
/**
 * @brief
 *  The main function. It creates a temperory copy of the hpcmgr_main script and execute it. 
 *  
 * @param[in] 
 *  The argv[] will be transferred to the hpcmgr script. Maximum 8 arguments.
 *
 * @return int
 *  0 if succeeded
 *  1 if failed. Detailed failure information would be printed out.
 *
 */
