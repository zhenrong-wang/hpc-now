...
echo -e "#!/usr/bin/expect\nset passwd $root_passwd\nspawn mysql_secure_installation" > /etc/dbconfig.sh
echo -e "expect {\n\t\t\"Enter current password\" { send \"\\\r\"; exp_continue }\n\t\t\"Y/n\" { send \"Y\\\r\"; exp_continue }" >> /etc/dbconfig.sh
echo -e "\t\t\"New password\" { send \"\$passwd\\\r\"; exp_continue }" >> /etc/dbconfig.sh
echo -e "\t\t\"Re-enter new password\" { send \"\$passwd\\\r\"; exp_continue }" >> /etc/dbconfig.sh
echo -e "\t\t\"Remove anonymous users\" { send \"Y\\\r\"; exp_continue }" >> /etc/dbconfig.sh
echo -e "\t\t\"Disallow root login remotely\" { send \"Y\\\r\"; exp_continue }" >> /etc/dbconfig.sh
echo -e "\t\t\"Remove test database and access to it\" { send \"Y\\\r\"; exp_continue }" >> /etc/dbconfig.sh
echo -e "\t\t\"Reload privilege tables now\" { send \"Y\\\r\" }\n}" >> /etc/dbconfig.sh
chmod +x /etc/dbconfig.sh
/etc/dbconfig.sh >> ${logfile} 2>&1
