...
# This script is used by 'hpcmgr' command to build *VASP-5.4.4* to HPC-NOW cluster.
...
mkdir -p ${app_root}vasp.5.4.4 && rm -rf ${app_root}vasp.5.4.4/*

echo -e "\n****************** ! DISCLAIMER ! ******************"
if [ $1 != 'remove' ]; then
  echo -e "* Place the source code to ${app_root}vasp.5.4.4.zip."
fi
...
if [ $1 = 'remove' ]; then
  ...
  rm -rf ${app_root}vasp.5.4.4
  echo -e "[ -INFO- ] Removing environment variables ..."
  if [ $current_user = 'root' ]; then
    sed -i '/< vasp5 >/d' $public_app_registry
    sed -i '/vasp5.env=/d' /etc/profile
  else
    sed -e "/< vasp5 > < ${current_user} >/d" $private_app_registry > /tmp/sed_${current_user}.tmp
    cat /tmp/sed_${current_user}.tmp > $private_app_registry
    rm -rf /tmp/sed_${current_user}.tmp
    sed -i '/vasp5.env=/d' $HOME/.bashrc
  fi
  echo -e "[ -INFO- ] VASP-5.4.4 has been removed successfully."
  exit 0
fi
...
if [ ! -f ${app_root}vasp.5.4.4.zip ]; then
  if [ $3 = 'local' ]; then
    /bin/cp -r ${url_pkgs}vasp.5.4.4.zip ${app_root}vasp.5.4.4.zip >> ${2}
  else
    wget ${url_pkgs}vasp.5.4.4.zip -O ${app_root}vasp.5.4.4.zip -o ${2}
  fi
fi
...
if [ $? -ne 0 ]; then
  echo -e "[ FATAL: ] Failed to unzip ${app_root}vasp.5.4.4.zip. Exit now."
  exit 3
fi
...
if [ $gcc_vnum -gt 9 ]; then
  sed -i 's/FFLAGS     = -w/FFLAGS     = -w -march=native -fPIC -fallow-argument-mismatch/g' ${app_extract_cache}vasp.5.4.4/makefile.include
else
  sed -i 's/FFLAGS     = -w/FFLAGS     = -w -march=native -fPIC/g' ${app_extract_cache}vasp.5.4.4/makefile.include
fi
sed -i "s@LIBDIR     = /opt/gfortran/libs/@LIBDIR     = ${lapack_root}@g" ${app_extract_cache}vasp.5.4.4/makefile.include
sed -i "s@SCALAPACK  = -L\$(LIBDIR) -lscalapack \$(BLACS)@SCALAPACK  = -L${scalapack_root} -lscalapack \$(BLACS)@g" ${app_extract_cache}vasp.5.4.4/makefile.include
sed -i "s@FFTW       ?= /opt/gfortran/fftw-3.3.4-GCC-5.4.1@FFTW       = ${fftw3_root}@g" ${app_extract_cache}vasp.5.4.4/makefile.include
sed -i "s@MPI_INC    = /opt/gfortran/openmpi-1.10.2/install/ompi-1.10.2-GFORTRAN-5.4.1/include@MPI_INC    = ${mpi_root}include@g" ${app_extract_cache}vasp.5.4.4/makefile.include
...
mkdir -p ${app_root}vasp.5.4.4/bin
rm -rf ${app_root}vasp.5.4.4/bin/*
/bin/cp -r build/std/vasp ${app_root}vasp.5.4.4/bin/vasp_std
/bin/cp -r build/ncl/vasp ${app_root}vasp.5.4.4/bin/vasp_ncl
/bin/cp -r build/gam/vasp ${app_root}vasp.5.4.4/bin/vasp_gam
echo -e "#! /bin/bash\nmodule purge" > ${app_root}vasp.5.4.4/vasp5.sh
echo -e "module load ${fftw3_env}\nmodule load ${scalapack_env}\nmodule load ${lapack_env}\nmodule load ${mpi_env}" >> ${app_root}vasp.5.4.4/vasp5.sh
if [ $systemgcc = 'false' ]; then
  echo -e "module load ${gcc_env}" >> ${app_root}vasp.5.4.4/vasp5.sh
fi
echo -e "export PATH=${app_root}vasp.5.4.4/bin:\$PATH" >> ${app_root}vasp.5.4.4/vasp5.sh
echo -e "echo -e \"VASP-5.4.4 is ready for running.\"" >> ${app_root}vasp.5.4.4/vasp5.sh

if [ $current_user = 'root' ]; then
  grep "alias vasp5.env" /etc/profile >> /dev/null 2>&1
  if [ $? -ne 0 ]; then
    echo -e "alias vasp5.env='source ${app_root}vasp.5.4.4/vasp5.sh'" >> /etc/profile
  fi
  echo -e "< vasp5 >" >> $public_app_registry
else
  grep "alias vasp5.env" $HOME/.bashrc >> /dev/null 2>&1
  if [ $? -ne 0 ]; then
    echo -e "alias vasp5.env='source ${app_root}vasp.5.4.4/vasp5.sh'" >> $HOME/.bashrc
  fi
  echo -e "< vasp5 > < ${current_user} >" >> $private_app_registry
fi
echo -e "[ -DONE- ] VASP-5.4.4 has been successfully installed to your cluster." 