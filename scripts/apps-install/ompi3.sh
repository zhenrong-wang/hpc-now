...
if [ $1 = 'install' ]; then
  ...
  if [ ! -f ${app_cache}ompi3.tar.gz ]; then
    if [ $3 = 'local' ]; then
      /bin/cp -r ${url_pkgs}prebuilds-9/ompi3.tar.gz ${app_cache}ompi3.tar.gz >> ${2}
    else
      wget ${url_pkgs}prebuilds-9/ompi3.tar.gz -O ${app_cache}ompi3.tar.gz -o ${2}
    fi
  fi
  echo -e "[ -INFO- ] Extracting the binaries and libraries ..."
  tar zvxf ${app_cache}ompi3.tar.gz -C ${app_root} >> ${2}
  if [ $? -ne 0 ]; then
    echo -e "[ FATAL: ] Failed to install OpenMPI-3.1.6. Please check the log file for details. Exit now."
    exit 1
  fi
  ...
  echo -e "[ -DONE- ] OpenMPI-3.1.6 has been installed."
  exit 0
fi
...
if [ ! -f ${app_cache}openmpi-3.1.6.zip ]; then
  if [ $3 = 'local' ]; then
    /bin/cp -r ${url_pkgs}openmpi-3.1.6.zip ${app_cache}openmpi-3.1.6.zip >> ${2}
  else
    wget ${url_pkgs}openmpi-3.1.6.zip -O ${app_cache}openmpi-3.1.6.zip -o ${2}
  fi
fi
cd ${app_cache} && unzip -o openmpi-3.1.6.zip -d ${app_extract_cache} >> ${2}
time_current=`date "+%Y-%m-%d %H:%M:%S"`
echo -e "[ STEP 2 ] $time_current Building libraries and binaries from the source packages ..."
cd ${app_extract_cache}openmpi-3.1.6
./configure --prefix=${app_root}ompi-3.1.6 --enable-mpi-cxx >> ${2} 2>&1
