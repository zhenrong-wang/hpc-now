...
if [ $1 = 'install' ]; then
  ...
  if [ ! -f ${app_cache}gcc9.tar.gz ]; then
    if [ $3 = 'local' ]; then
      /bin/cp -r ${url_pkgs}prebuilds-9/gcc9.tar.gz ${app_cache}gcc9.tar.gz >> ${2}
    else
...
if [ ! -f ${app_cache}gcc-9.5.0-full.tar.gz ]; then
  if [ $3 = 'local' ]; then
    /bin/cp -r ${url_pkgs}gcc-9.5.0-full.tar.gz ${app_cache}gcc-9.5.0-full.tar.gz >> ${2}
  else
    wget ${url_pkgs}gcc-9.5.0-full.tar.gz -O ${app_cache}gcc-9.5.0-full.tar.gz -o ${2}
  fi
fi
tar zvxf ${app_cache}gcc-9.5.0-full.tar.gz -C ${app_extract_cache} >> ${2}
cd ${app_extract_cache}gcc-9.5.0
./configure --prefix=${app_root}gcc-9.5.0 --enable-checking=release --enable-languages=c,c++,fortran --disable-multilib >> ${2} 2>&1
time_current=`date "+%Y-%m-%d %H:%M:%S"`
