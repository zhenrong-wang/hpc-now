...
logfile=/var/log/hpcmgr_install.log && echo -e "\n# $time_current INSTALLING MPICH - 3.2.1" >> ${logfile}
...
echo -e "# $time_current Started building MPICH-3.2.1." >> ${logfile}
if [ ! -d /opt/packs ]; then
  mkdir -p /opt/packs
fi
echo -e "[ STEP 1 ] $time_current Downloading and extracting source packages ..."
if [ ! -f /opt/packs/mpich-3.2.1.tar.gz ]; then
  wget -q ${URL_PKGS}mpich-3.2.1.tar.gz -O /opt/packs/mpich-3.2.1.tar.gz
fi
tar zvxf /opt/packs/mpich-3.2.1.tar.gz -C /opt/packs >> $tmp_log 2>&1
time_current=`date "+%Y-%m-%d %H:%M:%S"`
...
cd /opt/packs/mpich-3.2.1 && ./configure --prefix=/hpc_apps/mpich-3.2.1 --enable-cxx --enable-fortran=all >> $tmp_log 2>&1
make -j$NUM_PROCESSORS >> $tmp_log 2>&1
make install >> $tmp_log 2>&1
echo -e "#%Module1.0\nprepend-path PATH /hpc_apps/mpich-3.2.1/bin\nprepend-path LD_LIBRARY_PATH /hpc_apps/mpich-3.2.1/lib\nprepend-path C_INCLUDE_PATH /hpc_apps/mpich-3.2.1/include\nprepend-path CPLUS_INCLUDE_PATH /hpc_apps/mpich-3.2.1/include\n" > /etc/modulefiles/mpich-3.2.1
time_current=`date "+%Y-%m-%d %H:%M:%S"`
echo -e "[ -DONE- ] $time_current MPICH-3.2.1 has been built. "
echo -e "# $time_current MPICH-3.2.1 has been built." >> ${logfile}
