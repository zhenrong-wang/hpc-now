...
int get_static_conf_files(char* confdir, char* cloud_name, int code_loc_flag, char* url_code_root){
    ...
    if(strcmp(cloud_name,"alicloud")!=0&&strcmp(cloud_name,"aws")!=0&&strcmp(cloud_name,"azure")!=0&&strcmp(cloud_name,"baidu")&&strcmp(cloud_name,"gcp")!=0&&strcmp(cloud_name,"hwcloud")!=0&&strcmp(cloud_name,"qcloud")!=0&&strcmp(cloud_name,"volce")!=0){
        return 1;
    }
...
int get_tf_templates(char* confdir, char* stackdir, char* cloud_name, int code_loc_flag, char* url_code_root){
    char cmdline[CMDLINE_LENGTH]="";
    char filename_temp[FILENAME_LENGTH]="";
    char filename_new_temp[FILENAME_LENGTH]="";
    char url_code[DIR_LENGTH_EXT]="";
    ...
    batch_file_operation(stackdir,"hpc_stack*","","rm",0);
    ...
    if(file_exist_or_not(tf_conf)==1){
        ...
        if(code_loc_flag==1){
            snprintf(filename_temp,FILENAME_LENGTH-1,"%s%stf_prep.conf.v2",url_code,PATH_SLASH);
            if(cp_file(filename_temp,tf_conf,0)!=0){
                return 2;
            }
        }
        else{
    ...
    if(code_loc_flag==1){
        snprintf(filename_temp,FILENAME_LENGTH-1,"%s%shpc_stack_%s.base",url_code,PATH_SLASH,cloud_name);
        snprintf(filename_new_temp,FILENAME_LENGTH-1,"%s%shpc_stack.base",stackdir,PATH_SLASH);
        if(cp_file(filename_temp,filename_new_temp,0)!=0){
            return 2;
        }
    }
    else{
    ...
    if(code_loc_flag==1){
        snprintf(filename_temp,FILENAME_LENGTH-1,"%s%shpc_stack_%s.master",url_code,PATH_SLASH,cloud_name);
        snprintf(filename_new_temp,FILENAME_LENGTH-1,"%s%shpc_stack.master",stackdir,PATH_SLASH);
        if(cp_file(filename_temp,filename_new_temp,0)!=0){
            return 2;
        }
    }
    ...
    if(code_loc_flag==1){
        if(strcmp(cloud_name,"aws")==0){
            snprintf(filename_temp,FILENAME_LENGTH-1,"%s%shpc_stack_%s.compute.v2",url_code,PATH_SLASH,cloud_name);
        }
        else{
            snprintf(filename_temp,FILENAME_LENGTH-1,"%s%shpc_stack_%s.compute",url_code,PATH_SLASH,cloud_name);
        }
        snprintf(filename_new_temp,FILENAME_LENGTH-1,"%s%shpc_stack.compute",stackdir,PATH_SLASH);
        if(cp_file(filename_temp,filename_new_temp,0)!=0){
            return 2;
        }
    }
    ...
    if(code_loc_flag==1){
        snprintf(filename_temp,FILENAME_LENGTH-1,"%s%shpc_stack_%s.database",url_code,PATH_SLASH,cloud_name);
        snprintf(filename_new_temp,FILENAME_LENGTH-1,"%s%shpc_stack.database",stackdir,PATH_SLASH);
        if(cp_file(filename_temp,filename_new_temp,0)!=0){
            return 2;
        }
    }
    ...
    if(code_loc_flag==1){
        snprintf(filename_temp,FILENAME_LENGTH-1,"%s%shpc_stack_%s.natgw",url_code,PATH_SLASH,cloud_name);
        snprintf(filename_new_temp,FILENAME_LENGTH-1,"%s%shpc_stack.natgw",stackdir,PATH_SLASH);
        if(cp_file(filename_temp,filename_new_temp,0)!=0){
            return 2;
        }
    }
...
int cluster_init_conf(char* cluster_name, char* crypto_keyfile, int batch_flag_local, int code_loc_flag_local, char* url_code_root, int argc, char* argv[]){
    ...
    if(strcmp(cloud_flag,"CLOUD_A")==0){
        ...
        strcpy(default_region,"cn-beijing");
        strcpy(default_zone,"cn-beijing-a");
        strcpy(default_master_inst,"a8c16g");
        strcpy(default_compute_inst,"a4c8g");
        strcpy(default_os_image,"centoss9");
    }
...
int clear_if_failed(char* stackdir, char* confdir, char* vaultdir, int condition_flag){
    ...
    int error_flag=0;
    if(condition_flag==1){
        if(batch_file_operation(stackdir,"*","","rm",0)!=0){
            return 1;
        }
        return 0;
    }
        ...
        snprintf(filename_temp2,FILENAME_LENGTH-1,"%s%stf_prep.conf.failed",confdir,PATH_SLASH);
        ...
        if(batch_file_operation(DESTROYED_DIR,"*","","rm",0)!=0){
            error_flag++;
        }
        ...
        snprintf(filename_temp,FILENAME_LENGTH-1,"%s%stf_prep.conf",confdir,PATH_SLASH);
        snprintf(filename_temp2,FILENAME_LENGTH-1,"%s%stf_prep.conf.destroyed",confdir,PATH_SLASH);
        if(rename(filename_temp,filename_temp2)!=0){
            error_flag++;
        }
        return error_flag;
...
int generate_tf_files(char* stackdir){
    ...
    snprintf(filename_temp2,FILENAME_LENGTH-1,"%s%shpc_stack_database.tf",stackdir,PATH_SLASH);
    if(rename(filename_temp,filename_temp2)!=0){
        error_flag++;
    }
    snprintf(filename_temp,FILENAME_LENGTH-1,"%s%shpc_stack.master",stackdir,PATH_SLASH);
    snprintf(filename_temp2,FILENAME_LENGTH-1,"%s%shpc_stack_master.tf",stackdir,PATH_SLASH);
    if(rename(filename_temp,filename_temp2)!=0){
        error_flag++;
    }
    snprintf(filename_temp,FILENAME_LENGTH-1,"%s%shpc_stack.natgw",stackdir,PATH_SLASH);
    snprintf(filename_temp2,FILENAME_LENGTH-1,"%s%shpc_stack_natgw.tf",stackdir,PATH_SLASH);
    if(rename(filename_temp,filename_temp2)!=0){
        error_flag++;
    }
...
int aws_cluster_init(char* workdir, char* crypto_keyfile, int batch_flag_local, tf_exec_config* tf_run){
    ...
    if(get_opr_pubkey(SSHKEY_DIR,pubkey,1023)!=0){
        return -1;
    }
    ...
    for(i=0;i<init_info.node_num;i++){
        snprintf(filename_temp,FILENAME_LENGTH-1,"%s%shpc_stack.compute",stackdir,PATH_SLASH);
        snprintf(filename_temp2,FILENAME_LENGTH-1,"%s%shpc_stack_compute%d.tf",stackdir,PATH_SLASH,i+1);
        cp_file(filename_temp,filename_temp2,0);
        snprintf(string_temp,127,"compute%d",i+1);
        global_nreplace(filename_temp2,LINE_LENGTH_SMALL,"COMPUTE_NODE_N",string_temp);
        snprintf(string_temp,127,"comp%d",i+1);
        global_nreplace(filename_temp2,LINE_LENGTH_SMALL,"NUMBER",string_temp);
    }
    ...
    snprintf(filename_temp,FILENAME_LENGTH-1,"%s%shpc_stack_compute1.tf",stackdir,PATH_SLASH);
    snprintf(filename_temp2,FILENAME_LENGTH-1,"%s%scompute_template",stackdir,PATH_SLASH);
    cp_file(filename_temp,filename_temp2,0);
    ...
    remote_copy(workdir,crypto_keyfile,SSHKEY_DIR,filename_temp,"/hpc_data/cluster_data/.bucket.info","root","put","",0);
    ...
    snprintf(cluster_sshkey_dir,DIR_LENGTH-1,"%s%s.%s",SSHKEY_DIR,PATH_SLASH,init_info.cluster_id);
    mk_pdir(cluster_sshkey_dir);
    get_user_sshkey(init_info.cluster_id,"root","ENABLED",SSHKEY_DIR,crypto_keyfile);
    for(i=0;i<init_info.hpc_user_num;i++){
        snprintf(string_temp,127,"user%d",i+1);
        get_user_sshkey(init_info.cluster_id,string_temp,"ENABLED",SSHKEY_DIR,crypto_keyfile);
    }
...
int qcloud_cluster_init(char* workdir, char* crypto_keyfile, int batch_flag_local, tf_exec_config* tf_run){
    ...
    for(i=0;i<init_info.node_num;i++){
        ...
        snprintf(string_temp,127,"compute%d",i+1);
        global_nreplace(filename_temp2,LINE_LENGTH_SMALL,"COMPUTE_NODE_N",string_temp);
        global_nreplace(filename_temp2,LINE_LENGTH_SMALL,"RUNNING_FLAG","true");
    }
    generate_tf_files(stackdir);
    if(tf_execution(tf_run,"init",workdir,crypto_keyfile,0)!=0){
        clear_if_failed(stackdir,confdir,vaultdir,3);
...
int hwcloud_cluster_init(char* workdir, char* crypto_keyfile, int batch_flag_local, tf_exec_config* tf_run){
    ...
    for(i=0;i<init_conf.node_num;i++){
        ...
        snprintf(string_temp,127,"compute%d",i+1);
        global_nreplace(filename_temp2,LINE_LENGTH_SMALL,"COMPUTE_NODE_N",string_temp);
        global_nreplace(filename_temp2,LINE_LENGTH_SMALL,"RESOURCETAG",unique_cluster_id);
    }
    generate_tf_files(stackdir);
    if(tf_execution(tf_run,"init",workdir,crypto_keyfile,0)!=0){
        clear_if_failed(stackdir,confdir,vaultdir,3);
    ...
    remote_copy(workdir,crypto_keyfile,SSHKEY_DIR,filename_temp,"/root/hostfile","root","put","",0);
    sync_statefile(workdir,crypto_keyfile,SSHKEY_DIR);
    snprintf(cluster_sshkey_dir,DIR_LENGTH-1,"%s%s.%s",SSHKEY_DIR,PATH_SLASH,init_conf.cluster_id);
    mk_pdir(cluster_sshkey_dir);
    get_user_sshkey(init_conf.cluster_id,"root","ENABLED",SSHKEY_DIR,crypto_keyfile);
    for(i=0;i<init_conf.hpc_user_num;i++){
        snprintf(string_temp,127,"user%d",i+1);
        get_user_sshkey(init_conf.cluster_id,string_temp,"ENABLED",SSHKEY_DIR,crypto_keyfile);
    }
    print_cluster_init_done();
    create_local_tf_config(tf_run,stackdir);
    delete_decrypted_files(workdir,crypto_keyfile);
...
int baiducloud_cluster_init(char* workdir, char* crypto_keyfile, int batch_flag_local, tf_exec_config* tf_run){
    ...
    remote_copy(workdir,crypto_keyfile,SSHKEY_DIR,filename_temp,"/hpc_data/cluster_data/.bucket_creds/credentials","root","put","",0);
...
int gcp_cluster_init(char* workdir, char* crypto_keyfile, int batch_flag_local, tf_exec_config* tf_run){
    ...
    remote_copy(workdir,crypto_keyfile,SSHKEY_DIR,filename_temp,"/hpc_data/cluster_data/.bucket_key.json","root","put","",0);
    snprintf(filename_temp,FILENAME_LENGTH-1,"%s%sbucket_info.txt",vaultdir,PATH_SLASH);
...
int volce_cluster_init(char* workdir, char* crypto_keyfile, int batch_flag_local, tf_exec_config* tf_run){
    char cluster_id_from_workdir[CLUSTER_ID_LENGTH_MAX_PLUS]="";
    if(get_cluster_nname(cluster_id_from_workdir,CLUSTER_ID_LENGTH_MAX_PLUS,workdir)!=0){
        return -3;
    }
    ...
    printf("[ STEP 1 ] Creating initialization files now ...\n");
    if(get_tf_templates(confdir,stackdir,"volce",code_loc_flag_var,url_code_root_var)!=0){
        printf(FATAL_RED_BOLD "[ FATAL: ] Failed to download/copy necessary file(s)." RESET_DISPLAY "\n");
        clear_if_failed(stackdir,confdir,vaultdir,1);
        return 2;
    }
    ...
    snprintf(string_temp,127,"%d",init_conf.master_bandwidth);
    global_nreplace(filename_temp,LINE_LENGTH_SMALL,"MASTER_BANDWIDTH",string_temp);
    if(strcmp(init_conf.os_image_raw,"centoss9")==0||strcmp(init_conf.os_image_raw,"centos7")==0){
        global_nreplace(filename_temp,LINE_LENGTH_SMALL,"OS_IMAGE",init_conf.os_image_raw);
    }
    else{
        snprintf(string_temp,127,"\"%s\"",init_conf.os_image_raw);
        global_nreplace(filename_temp,LINE_LENGTH_SMALL,"data.volcengine_images.OS_IMAGE.images[0].image_id",string_temp);
    }
    global_nreplace(filename_temp,LINE_LENGTH_SMALL,"PUBLIC_KEY",pubkey);
    for(i=0;i<init_conf.hpc_user_num;i++){
        snprintf(line_temp,LINE_LENGTH-1,"echo -e \"username: user%d ${var.user%d_passwd} STATUS:ENABLED\" >> /root/user_secrets.txt",i+1,i+1);
    ...
    tf_execution(tf_run,"apply",workdir,crypto_keyfile,2);
    if(tf_execution(tf_run,"apply",workdir,crypto_keyfile,3)!=0){
        printf(GENERAL_BOLD "[ -INFO- ]" RESET_DISPLAY " Rolling back and exit now ...\n");
        if(tf_execution(tf_run,"destroy",workdir,crypto_keyfile,1)==0){
            delete_decrypted_files(workdir,crypto_keyfile);
            clear_if_failed(stackdir,confdir,vaultdir,3);
    ...
    getstate(workdir,crypto_keyfile);
    snprintf(filename_temp,FILENAME_LENGTH-1,"%s%sterraform.tfstate",stackdir,PATH_SLASH);
    find_and_nget(filename_temp,LINE_LENGTH_SMALL,"\"volcengine_tos_bucket\"","","",20,"\"bucket_name\"","","",'\"',4,bucket_id,32);
    find_and_nget(filename_temp,LINE_LENGTH_SMALL,"access_key","","",40,"\"id\":","","",'\"',4,bucket_ak,256);
    find_and_nget(filename_temp,LINE_LENGTH_SMALL,"access_key","","",40,"\"secret\":","","",'\"',4,bucket_sk,256);
    printf("[ STEP 3 ] Remote executing now, please wait %d seconds for this step.\n",GENERAL_SLEEP_TIME);
    for(i=0;i<GENERAL_SLEEP_TIME;i++){
        printf("[ -WAIT- ] Still need to wait %d seconds ... \r",GENERAL_SLEEP_TIME-i);
        fflush(stdout);
