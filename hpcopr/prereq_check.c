...
int command_parser(int argc, char** argv, char* command_name_prompt, char* workdir, char* cluster_name, char* user_name, char* cluster_role){
    ...
    if(strcmp(cu_flag,"UNAME")==0||strcmp(cu_flag,"CNAME")==0){
        ...
        if(decryption_status(workdir)!=0){
            ...
            printf("|          Please run " WARN_YELLO_BOLD "hpcopr encrypt --all -b" RESET_DISPLAY FATAL_RED_BOLD " or" RESET_DISPLAY WARN_YELLO_BOLD " hpcopr encrypt -c %s -b" RESET_DISPLAY FATAL_RED_BOLD" !!!" RESET_DISPLAY "\n",cluster_name);
        }
