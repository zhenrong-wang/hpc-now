...
//    char crypto_keyfile[FILENAME_LENGTH]="";
    ...
    sprintf(cmdline,"cd %s && %s init >> %s/tf_prep.log 2>%s && %s apply >> %s/tf_prep.log 2>%s",stackdir,tf_exec,stackdir,logfile,tf_exec,stackdir,logfile);
    system(cmdline);
    reset_string(cmdline);
...
int check_and_install_prerequisitions(char* current_command){
    ...
    if(file_exist_or_not("/usr/.hpc-now/.bin/terraform.exe")!=0){
        ...
        flag=system("curl https://hpc-now-1308065454.cos.ap-guangzhou.myqcloud.com/terraform/terraform -o /usr/.hpc-now/.bin/terraform.exe -s");
        if(flag!=0){
            printf("+-----------------------------------------------------------------------------------+\n");
    ...
    if(file_exist_or_not("/usr/.hpc-now/.bin/now-crypto.exe")!=0){
        ...
        flag=system("curl https://hpc-now-1308065454.cos.ap-guangzhou.myqcloud.com/utils/now-crypto -o /usr/.hpc-now/.bin/now-crypto.exe -s");
        if(flag!=0){
            printf("+-----------------------------------------------------------------------------------+\n");
            printf("[ FATAL: ] Failed to download or install necessary tools. Please contact            |\n");
            printf("|          info@hpc-now.com for support. Exit now.                                  |\n");
    ...
    if(file_exist_or_not(filename_temp)!=0){
        ...
        if(file_exist_or_not(filename_temp)!=0){
            ...
            sprintf(cmdline,"curl https://hpc-now-1308065454.cos.ap-guangzhou.myqcloud.com/terraform/terraform-provider-alicloud_v%s.tar.xz -o %s -s",ali_plugin_version,filename_temp);
            system(cmdline);
        }
    ...
    if(file_exist_or_not(filename_temp)!=0){
        ...
        if(file_exist_or_not(filename_temp)!=0){
            ...
            sprintf(cmdline,"curl https://hpc-now-1308065454.cos.ap-guangzhou.myqcloud.com/terraform/terraform-provider-tencentcloud_v%s.tar.xz -o %s -s",qcloud_plugin_version,filename_temp);
            system(cmdline);
        }
        sprintf(cmdline,"tar xf %s -C %s >> /dev/null 2>&1",filename_temp,dirname_temp);
        system(cmdline);   
    ...
    if(file_exist_or_not(filename_temp)!=0){
        ...
        if(file_exist_or_not(filename_temp)!=0){
            ...
            sprintf(cmdline,"curl https://hpc-now-1308065454.cos.ap-guangzhou.myqcloud.com/terraform/terraform-provider-aws_v%s_x5.tar.xz -o %s -s",aws_plugin_version,filename_temp);
            system(cmdline);
        }
        sprintf(cmdline,"tar xf %s -C %s >> /dev/null 2>&1",filename_temp,dirname_temp);
        system(cmdline);   
...
int expiration_date_calc(int real_year, int real_month, int real_day, int* expiration_date, int period_flag){
  if(period_flag==0){
    if(real_month==1||real_month==3||real_month==5||real_month==7||real_month==8||real_month==10||real_month==12){
      if((real_day+7)>31){
        expiration_date[2]=real_day+7-31;
        if((real_month+1)>12){
          expiration_date[1]=1;
          expiration_date[0]=real_year+1;
        }
        else{
          expiration_date[1]=real_month+1;
          expiration_date[0]=real_year;
        }
      }
      else{
        expiration_date[2]=real_day+7;
        expiration_date[1]=real_month;
        expiration_date[0]=real_year;
      }
    }
      ...
      expiration_date[0]=real_year;
      if((real_day+7)>30){
        expiration_date[2]=real_day+7-30;
        expiration_date[1]=real_month+1;
      }
      else{
        expiration_date[2]=real_day+7;
    ...
    else if(real_month==2){
      expiration_date[0]=real_year;
      if(real_year%4==0){
        if((real_day+7)>29){
          expiration_date[1]=3;
          expiration_date[2]=real_day+7-29;
        }
        else{
          expiration_date[1]=2;
          expiration_date[2]=real_day+7;
        ...
        if((real_day+7)>28){
          expiration_date[1]=3;
          expiration_date[2]=real_day+7-28;
        }
        else{
          expiration_date[1]=2;
          expiration_date[2]=real_day+7;
    ...
    if((real_month+1)>12){
      expiration_date[0]=real_year+1;
      expiration_date[1]=real_month+1-12;
      expiration_date[2]=real_day;
    }
    else{
      expiration_date[0]=real_year;
      expiration_date[1]=real_month+1;
      if(real_month==3||real_month==5||real_month==8||real_month==10){
        if(real_day==31){
          expiration_date[2]=30;
        }
        else{
          expiration_date[2]=real_day;
        }
      }
      else if(real_month==1){
        if(real_year%4==0&&real_day>29){
          expiration_date[2]=29;
        }
        else if(real_year%4!=0&&real_day>28){
