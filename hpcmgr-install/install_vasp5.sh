...
echo -e "[ -INFO- ] Software: VASP-5.4.4\n"
echo -e "**************** IMPORTANT! ****************"
...
logfile=/var/log/hpcmgr_install.log && echo -e "\n# $time_current INSTALLING VASP-5.4.4" >> ${logfile}
APP_ROOT=/hpc_apps
...
if [ ! -f /opt/packs/vasp.5.4.4.zip ]; then
  wget ${URL_PKGS}vasp.5.4.4.zip -q -O /opt/packs/vasp.5.4.4.zip
fi
rm -rf $APP_ROOT/vasp.5.4.4 && unzip -q /opt/packs/vasp.5.4.4.zip -d $APP_ROOT
echo -e "[ -INFO- ] Building binaries from the source code ... This step usually take minutes..."
cd $APP_ROOT/vasp.5.4.4 && /bin/cp arch/makefile.include.linux_gnu makefile.include
if [ $gcc_vnum -gt 10 ]; then
  sed -i 's/FFLAGS     = -w/FFLAGS     = -w -march=native -fPIC -fallow-argument-mismatch/g' $APP_ROOT/vasp.5.4.4/makefile.include
else
  sed -i 's/FFLAGS     = -w/FFLAGS     = -w -march=native -fPIC/g' $APP_ROOT/vasp.5.4.4/makefile.include
fi
sed -i 's@LIBDIR     = /opt/gfortran/libs/@LIBDIR     = /hpc_apps/lapack-3.11@g' $APP_ROOT/vasp.5.4.4/makefile.include
sed -i 's@SCALAPACK  = -L$(LIBDIR) -lscalapack $(BLACS)@SCALAPACK  = -L/hpc_apps/scalapack -lscalapack $(BLACS)@g' $APP_ROOT/vasp.5.4.4/makefile.include
sed -i 's@FFTW       ?= /opt/gfortran/fftw-3.3.4-GCC-5.4.1@FFTW       = /hpc_apps/fftw3@g' $APP_ROOT/vasp.5.4.4/makefile.include
echo -e "CPP_OPTIONS += -DLAPACK36" >> $APP_ROOT/vasp.5.4.4/makefile.include
echo -e "! routines replaced in LAPACK >=3.6\n#ifdef LAPACK36\n#define DGEGV DGGEV\n#endif" >> $APP_ROOT/vasp.5.4.4/src/symbol.inc
cd $APP_ROOT/vasp.5.4.4 && rm -rf build/*
...
if [ $systemgcc = 'true' ]; then
  echo -e "#! /bin/bash\nmodule purge\nmodule load $mpi_version\nmodule load fftw3\nexport PATH=/hpc_apps/vasp.5.4.4/bin:\$PATH\necho \"VASP is ready for running\"" > $APP_ROOT/vasp.5.4.4/vasp5.sh
else
  echo -e "#! /bin/bash\nmodule purge\nmodule load $mpi_version\nmodule load $gcc_v\nmodule load fftw3\nexport PATH=/hpc_apps/vasp.5.4.4/bin:\$PATH\necho \"VASP is ready for running\"" > $APP_ROOT/vasp.5.4.4/vasp5.sh
fi
cat /etc/profile | grep "alias vasp5=" >> /dev/null 2>&1
if [ $? -ne 0 ]; then
  echo -e "alias vasp5='source $APP_ROOT/vasp.5.4.4/vasp5.sh'" >> /etc/profile
fi
source /etc/profile
